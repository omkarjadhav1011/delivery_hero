...
- [ ] T6 OpenAPI check comparing the generated document with `docs/openapi.json` (written to `backend/target/openapi.json` on a difference), and `docs/openapi.json` created from it, in `backend/src/test/java/app/deliveryhero`, test first: `OpenApiIT`, source: DEC-175, document 18 section 8.5
