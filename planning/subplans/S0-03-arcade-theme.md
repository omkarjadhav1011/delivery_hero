...
- 2026-09-25: T6 done. There are three shells.
  - `PhoneShell`: a banner with the brand (or the top bar once US-16 adds it), `main` with the `h1`, and bottom actions. It has no fixed widths and keeps to the safe area.
  - `ProjectorShell`: a full-viewport 16:9 page with a 12-column grid, the wall on 8 columns and an `aside` on 4.
  - `AdminShell`: a banner and a navigation in the A-02 order with `aria-current`, and no navigation on the login page.
  They replace `PlaceholderPage` on all 13 routes. Log out is left as a `TODO(US-49)`, because no story or copy in document 04 names it yet. One new string, `copy.admin.navLabel` ("Admin sections"), is worded in the copy deck's style (DI-21) for the owner's review. Headings that are sentences use the text font, and the brand uses the pixel font (section 5.3). 51 tests pass; format, lint, typecheck and the build are clean.
- 2026-09-25: T7 done. `security-privacy.spec.ts` (AC-EN08-02) logs every font, image and script request on `/`, `/join/`, `/screen/`, `/admin/login/` and `/admin/`, beside the shared outside-request blocker. It requires at least one font and one script, so the log can't be empty by accident, and checks that each request stays on the game's own address. A probe image from `example.com` fails it, and the CSP listener too. 5 of 5 pass on the e2e stack (ports 8090 and 5433).
- 2026-09-25: T8 done for the phone pages that exist, `/` and `/join/`. `accessibility.spec.ts` (AC-EN08-03) runs at 320 × 640, and again with the root font at 200%. It checks that the page's `scrollWidth` is at most its `clientWidth`, and that every control scrolls into view inside the 320 px width. The 320 px run also runs axe. A probe 400 px button fails it (360 > 320). 4 of 4 pass. The wide-code-line step waits for a page that shows `CodeBlock` (the code task screen, US-26); until then the `CodeBlock` unit test covers its own scroll region.
- 2026-09-25: T2 in part. The README's "Credits and licenses" table now credits Press Start 2P, as its `OFL.txt` names the authors, and Pixelarticons 2.4.1 (MIT), each with the path of its license file. The Pixelarticons license is in `frontend/src/ui/icons/LICENSE`, beside the path data, rather than in `frontend/public/`, because the icons are drawn inline. `markdownlint-cli2 README.md` is clean. T2 stays open for the pixel-art pack row (T1, R-11).
- 2026-09-25: Review fixes (frontend-reviewer, spec-guardian).
  - `AdminShell` marks the current section whether or not the path ends in `/`; this was checked on the e2e stack for `/admin/tasks/` and `/admin/tasks/edit/`.
  - The login page has no header, as the A-01 wireframe shows.
  - The pixel-font guard in `theme.test.ts` also scans template literals, and a `text-sm` probe in `SpeechBubble` now fails it.
  - The reduced-motion comment now says what the rule does: it ends CSS animations and leaves transitions, so fades are built as transitions.
...
- 2026-09-25: In review: pull request #11 is open. T1 (the pixel-art pack), T2 (its README row), T5 (the art) and T9 (MAN-02) are still open.
