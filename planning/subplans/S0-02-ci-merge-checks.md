...
- [x] T1 Align `ci.yml` with Appendix F and section 9.5: the `changes` job's filters (`backend/`, `seed/`, `contracts/` for the backend; `frontend/`, `contracts/` for the frontend; `backend/`, `frontend/`, `deploy/`, `seed/` for end-to-end), the always-on `repo-checks`, `concurrency` with `cancel-in-progress`, Maven and npm caches, and the Playwright browser cache that section 9.5 names, in `.github/workflows/ci.yml`, test first: procedure (a scratch pull request changing only `docs/` runs only `repo-checks`; a second push cancels the first run), source: DEC-183, DEC-176 (shared), NFR-43, document 13 sections 9.5 and Appendix F
- [x] T2 Add a pinned actionlint step to `repo-checks` (a pinned release or image digest, not `latest`), in `.github/workflows/ci.yml`, test first: actionlint run locally on both workflows, and a scratch commit with a bad `needs:` reference fails the step, source: DEC-177, DI-22, document 13 section 5.1
...
- [x] T9 Owner: approve recording the doc fixes for DI-22 (an actionlint step in Appendix F, the section 9.5 diagram's path filters) at document 13's next revision (owner approval needed: changes docs/) (Owner approval needed: changes docs/), test first: none, source: DI-22, DEC-177, document 13 section 9.5
...
- 2026-09-25: T1 checked locally with no change. `ci.yml` equals Appendix F apart from the SHA pins and `GITLEAKS_VERSION`. The filters, the always-on `repo-checks`, `concurrency` with cancel, and the Maven and npm caches all match. The owner chose to leave Playwright's browser uncached, recorded in DI-22. Open until a GitHub run proves that a docs-only PR runs only `repo-checks` and a new push cancels the earlier run.
- 2026-09-25: T2 done locally. A "Workflows" step runs `rhysd/actionlint:1.7.12@sha256:b1934ee5…`. Both workflows pass, and a probe with `needs: [changes, backend, frontnd]` fails with exit 1 ("job e2e needs job frontnd which does not exist"). Open until a red run on GitHub.
- 2026-09-25: T4 checked locally. `.github/dependabot.yml` equals Appendix E. Majors are ignored only for Maven and npm, not Docker, so the temurin 25 PR (#1) appeared; recorded as DI-30. Open until the Dependabot action PRs are seen bumping SHA pins rather than tags on GitHub (needs `gh auth login` or the owner).
- 2026-09-25: T5 done. An untested `engine.CoverageProbe` (a throwaway, removed) fails `./mvnw -B verify` with "Rule violated for package app.deliveryhero.engine: lines covered ratio is 0.00, but expected minimum is 0.80". The unit (`prepare-agent`) and integration (`prepare-agent-integration`) agents write the same `target/jacoco.exec`, so the rule counts both combined. No change to `pom.xml`.
- 2026-09-25: T7 done. `deploy.yml` equals Appendix B.6 apart from the SHA pins and the approved `planning/**` and `.claude/**` ignores (DI-01), with `concurrency: deploy-production` and `cancel-in-progress: false`. Its build commands, run locally on throwaway probes (all removed):
  - `./mvnw -B package` fails on a failing unit test, a Spotless violation and an Error Prone `DeadException`.
...
- 2026-09-26: T4 done. `.github/dependabot.yml` is identical to Appendix E. Dependabot's action PRs #2 to #5 bumped tags (`actions/checkout@v5` to `@v7`), but they came before S0-01's SHA pins and were closed. None has come since, so the SHA-pin update is confirmed on the first Monday run (Mon 28 Sep, with T8). Found on the way: PR #1 (merged 2026-09-25) moved `deploy/backend/Dockerfile` to `eclipse-temurin:25-jre`, against DEC-123 and AD-01 (Java 21), because Appendix E's docker entry ignores no majors (DI-48).
- 2026-09-26: The owner chose Java 21, as DEC-123 says: `deploy/backend/Dockerfile` is back on `eclipse-temurin:21-jre`, and the docker entry in `.github/dependabot.yml` now ignores major versions like Maven and npm. The file is now one block ahead of Appendix E (DI-48).
- 2026-09-26: T1 done. The e2e job caches `~/.cache/ms-playwright`, keyed on the Playwright version in `package-lock.json` (`actions/cache` v6.1.0, pinned by SHA). Proofs:
  - PR #15 (planning files only) ran only `changes` and `repo-checks` (https://github.com/omkarjadhav1011/delivery_hero/actions/runs/36217912360).
  - On scratch PR #17, a second push 8 seconds after opening cancelled the first run (36218547461, cancelled).
  - PR #16's e2e job missed and saved `playwright-Linux-1.63.0` (284 MB, run 36218503005 attempt 1), and hit it on the re-run (attempt 2, green).
- 2026-09-26: T2 done. Pinned actionlint 1.7.12 passes on both workflows locally, and a bad `needs:` fails it locally (`job-needs`). On GitHub a bad `needs:` rejects the whole workflow before any job runs, so the scratch commit used an undefined `needs.changes.outputs.backendz` instead: `repo-checks` failed at the Workflows step (https://github.com/omkarjadhav1011/delivery_hero/actions/runs/36218849376).
- 2026-09-26: T6 done on scratch PR #17 (draft, closed unmerged, branch deleted). Each failure was reverted in the next commit, and each run was red:
  - Bad title "scratch merge gate": `repo-checks` failed at the title step (36218558890).
  - Failing unit test: `backend` failed at `NamesTest.scratchFailingTest` (36218940825).
  - Spotless violation in `Names.java`: `backend` failed at `spotless:check` (36219104430).
  - Error Prone `DeadException` in `Names.java`: `backend` failed with a compilation error (36219197097).
- 2026-09-26: Actuals. 10:04 to 10:25 (about 25 min), plus about 10 CI minutes across PRs #16 and #17. Main-session tokens aren't measured (an estimate of about 150k); ops-reviewer used about 22k. PR #16 open; the status stays In progress for the owner tasks T8 and T9.
- 2026-09-26: T9 done. The owner approved recording the document 13 fixes for DI-22 (Appendix F's actionlint step, the section 9.5 diagram's path filters) and DI-48 (Appendix E's docker major-version ignore) for its next revision. Both stay Open in `doc-issues.md` until that revision, which is its own approved change. Only T8 (Mondays, from 28 Sep) is left.
