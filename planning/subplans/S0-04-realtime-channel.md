...
- [ ] T8 Local stack check through Nginx: a STOMP client connects to `ws://localhost:8080/ws` with a test token registered in the `dev` profile only, heartbeats flow, and a wrong origin is refused, in `deploy/docker-compose.local.yml` only if the proxy needs a fix, test first: procedure (local stack; the `/ws` location in `deploy/nginx/snippets/routes.conf` upgrades the connection), source: AC-EN04-01 (shared with S0-05's end-to-end run), DEC-67, DEC-127, document 16 Appendix B.5
