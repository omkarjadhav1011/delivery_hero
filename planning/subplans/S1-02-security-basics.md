...
- [x] T8 `security-privacy` spec steps 1 and 3 on the local e2e stack: headers on every page and API response, and every request goes to the game's own address, in `frontend/e2e/security-privacy.spec.ts`, test first: the spec itself, source: E2E-07, AC-EN06-01, AC-EN08-02 (shared)
...
- 2026-09-26: T4 done. `RealtimeController` maps `/games/{gameId}/answer`: it stamps `receivedAt` first, drops answers over `RateLimiter.ANSWER` with no reply, then queues `SubmitAnswer`. Added the LLD 5.2 `AnswerPayload` family in `common` (so `scoring` can use it), read by `kind` as API 8.4 gives it; `OrderAnswer` keeps the LLD field name and reads API 8.4's `itemIndexes`. `GameSession` accepts `SubmitAnswer` and does nothing until US-27 (no state accepts answers yet). The owner approved a unit test (`RealtimeControllerTest`, fixed clock) instead of `SecurityIT` for the answer limit. Tests: `RealtimeControllerTest` (3), `AnswerPayloadTest` (4); `ArchitectureTest`, `StompConnectionIT` and `PlayerStateIT` still pass.
- 2026-09-26: T7 done. DS-05 is a shared fixture (`frontend/e2e/fixtures/ds05.ts`) with no Playwright import, so Vitest and the specs use the same strings. `CodeBlock.test.tsx` and `SpeechBubble.test.tsx` render it: literal text, no `script` or `img` element, `alert` never called. ESLint `react/no-danger` was already `error`, and nothing in `app`, `src` or `e2e` uses `innerHTML`.
