...
- [ ] T1 Store the player token after a successful join under `dh.token.<code>` in local storage and read it back on load (guarding storage errors), in `frontend/src/player/session.ts`, test first: `session.test.ts` AC-US05-01, source: AC-US05-01, FR-007, BR-17, document 11 section 7.2
