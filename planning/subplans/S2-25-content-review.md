...
- [ ] T5 Owner: after the merge deploys, load the reviewed pool on production with the seed command (the loader upserts by key, and refuses while a game is open), and check the counts it prints, test first: none, source: R-10, FR-075 (shared), document 16 section 9.5, document 08 section 5.10 [Blocked: waiting for Q-01]
