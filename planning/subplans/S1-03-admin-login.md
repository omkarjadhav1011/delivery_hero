...
- 2026-09-26: T5 done. `LoginScreen` (A-01, `src/admin/screens/`) has a labelled password field (`autocomplete="current-password"`) and "Log in"; it calls `GET /api/admin/session` for the CSRF cookie, then posts the form, and opens `/admin/`. It shows "That password didn't work." or the rate-limit message, clears the field and moves focus back to it, following `JoinScreen` (there's no `LiveAnnouncer` yet). `http.ts` posts URL-encoded forms and calls a handler on `UNAUTHENTICATED` from any admin call except login; `AdminShell` registers it with the router, so an ended session goes to login. The A-02 header's "Log out" (the shell's US-49 TODO) posts logout and returns to login. There's no copy for a login that can't reach the server: DI-54. Frontend: format, lint, typecheck, 118 tests and the build pass.
