...
- [ ] T4 Login and security logs: `LOGIN_SUCCEEDED`, `LOGIN_FAILED` and `RATE_LIMITED` carry the IP and limit only; the password never reaches a log line, in `app.deliveryhero.security`, test first: `SecurityIT` AC-US49-05 log capture during a failed and a successful login finds no password, source: AC-US49-05, NFR-14, DEC-104 (shared), LLD 5.14
...
- [ ] T7 Owner: MAN-08 on production: inspect `/opt/delivery-hero/.env` for a bcrypt hash with cost 12 or more and search the logs for the password; record the result, test first: none, source: MAN-08, AC-US49-05, NFR-14, R-05, document 15 section 12 [Blocked: waiting for Q-01]
